...
export default function Swap({ config, address, setisLoading, setTxupdate, setisError, setErrMsg, options, inputStyle, jcExchange, exchangeABI, juExchange, exchangeJulpABI, jcSwap, swapABI, juSwap, swapJulpABI, cmjToken, jusdtToken, erc20Abi, jbcBalance, cmjBalance, cmjBalanceFull, jusdtBalance, jusdtBalanceFull, jbcReserv, cmjReserv, jbcJuReserv, jusdtJuReserv, priceTHB }) {
    ...
    const handleSwap = useDebouncedCallback(async () => {
        setCmjBought('Getting a Quote...')
        const amount = inputSwap
    ...
    const handleSwap2 = useDebouncedCallback(async () => {
        setJbcBought('Getting a Quote...')
        const amount = inputSwap     
        const _value = amount !== "" ? ethers.utils.parseEther(String(amount)) : 0
        const _reserveJbc = await getBalance(config, { address: jcExchange, })
        const _reserveCmj = await readContract(config, {
    ...
    const handleSwap3 = useDebouncedCallback(async () => {
        setJusdtJuBought('Getting a Quote...')
        const amount = inputSwap
        const _value = amount !== "" ? ethers.utils.parseEther(String(amount)) : 0
    ...
    const handleSwap4 = useDebouncedCallback(async () => {
        setJbcJuBought('Getting a Quote...')
        const amount = inputSwap
        const _value = amount !== "" ? ethers.utils.parseEther(String(amount)) : 0
        const _reserveJbc = await getBalance(config, { address: juExchange, })
        const _reserveJusdt = await readContract(config, {
            ...
            <div style={{marginTop: "20px", padding: "20px 0", height: "450px", width: "350px", boxShadow: "6px 6px 0 #00000040"}} className="nftCard">
                ...
                <div style={{width: "85%", display: "flex", justifyContent: "space-between"}}>
                    <div style={{display: "flex"}}>
                        {(swapMode === 0 || swapMode === 3) && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreih6o2px5oqockhsuer7wktcvoky36gpdhv7qjwn76enblpce6uokq" alt="$JBC" />}
                        {swapMode === 1 && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreiabbtn5pc6di4nwfgpqkk3ss6njgzkt2evilc5i2r754pgiru5x4u" alt="$CMJ" />}
                        {swapMode === 2 && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreif3vllg6mwswlqypqgtsh7i7wwap7zgrkvtlhdjoc63zjm7uv6vvi" alt="$JUSDT" />}
                ...
                {(swapMode === 0 || swapMode === 1) &&
                    <div style={{width: "85%", display: "flex", justifyContent: "space-between"}}>
                        <input
                            ...
                            onChange={(e) => {
                                if (swapMode === 0) {
                                    setInputSwap(e.target.value)
                                    handleSwap()
                                    setInterval(handleSwap, 12000)
                                } else {
                                    setInputSwap(e.target.value)
                                    handleSwap2()
                                    setInterval(handleSwap2, 12000)
                                }
                            }}
                            value={inputSwap}
                            type='number'
                        ...
                        <div 
                            ...
                            onClick={() => {
                                if (swapMode === 0) {
                                    setInputSwap(jbcBalance - 0.0009)
                                    handleSwap()
                                    setInterval(handleSwap, 12000)
                                } else {
                                    setInputSwap(cmjBalanceFull)
                                    handleSwap2()
                                    setInterval(handleSwap2, 12000)
                                }
                            }}
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                if (swapMode === 3) {
                                    setInputSwap(e.target.value)
                                    handleSwap3()
                                    setInterval(handleSwap3, 12000)
                                } else {
                                    setInputSwap(e.target.value)
                                    handleSwap4()
                                    setInterval(handleSwap4, 12000)
                                }
                            }}
                            value={inputSwap}
                            type='number'
                ...
                <div style={{width: "85%", display: "flex", justifyContent: "space-between"}}>
                    <div style={{display: "flex"}}>
                        {swapMode === 0 && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreiabbtn5pc6di4nwfgpqkk3ss6njgzkt2evilc5i2r754pgiru5x4u" alt="$CMJ" />}
                        {(swapMode === 1 || swapMode === 2) && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreih6o2px5oqockhsuer7wktcvoky36gpdhv7qjwn76enblpce6uokq" alt="$JBC" />}
                        {swapMode === 3 && <img style={{width: "38px", height: "38px", marginRight: "2.5px"}} src="https://gateway.pinata.cloud/ipfs/bafkreif3vllg6mwswlqypqgtsh7i7wwap7zgrkvtlhdjoc63zjm7uv6vvi" alt="$JUSDT" />}
                        {swapMode === 0 &&
                            <Select
